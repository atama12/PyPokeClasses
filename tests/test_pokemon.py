...
class TestPokemon:
    ...
    def test_pokemon_init(self,mock_make_request):
        # モックレスポンスの設定
        mock_make_request.return_value.status_code = 200
        mock_make_request.return_value = mock_pokemon_response_data

        # インスタンス生成とテスト
        instance = Pokemon(1)
        assert instance.id == 35
        assert instance.name == "clefairy"
        assert instance.base_experience == 113
        assert instance.height == 6
        assert instance.is_default == True
        assert instance.order == 56
        assert instance.weight == 75
        assert instance.abilities[0].is_hidden == True
        assert instance.abilities[0].slot == 3
        assert instance.abilities[0].ability.name == "friend-guard"
        assert instance.abilities[0].ability.url == "https://pokeapi.co/api/v2/ability/132/"
        assert instance.forms[0].name == "clefairy"
        assert instance.forms[0].url == "https://pokeapi.co/api/v2/pokemon-form/35/"
