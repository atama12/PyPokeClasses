...
from pokeAPI_Models.Encounters import EncounterMethod
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Encounter/files/encounter-method.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = EncounterMethod(1)
    test2 = EncounterMethod(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ...
    ("names[0].name","Im hohen Gras oder in einer H\u00f6hle laufen"),
    ("order",1),
