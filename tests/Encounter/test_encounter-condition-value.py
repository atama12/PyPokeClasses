...
def mock_response_data():
    with open('tests/Encounter/files/encounter-condition-value.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = EncounterConditionValue(1)
    test2 = EncounterConditionValue(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("condition.name","swarm"),
    ("condition.url","https://pokeapi.co/api/v2/encounter-condition/1/"),
    ("id",1),
    ("name","swarm-yes"),
...
def test_data_check(mock_make_request, property_name, expected_value, mock_response_data):
    ...
    test_data = EncounterConditionValue(1)
