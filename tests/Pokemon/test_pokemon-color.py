...
from PyPokeClasses.Pokemon import PokemonColors
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Pokemon/files/pokemon-color.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = PokemonColors(1)
    test2 = PokemonColors(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ...
    ("names[0].name","\u304f\u308d\u3044\u308d"),
    ("pokemon_species[0].name","snorlax"),
    ("pokemon_species[0].url","https://pokeapi.co/api/v2/pokemon-species/143/"),
])
