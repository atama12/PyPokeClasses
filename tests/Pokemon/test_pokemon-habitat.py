...
from pokeAPI_Models.Pokemon import PokemonHabitats
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Pokemon/files/pokemon-habitat.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = PokemonHabitats(1)
    test2 = PokemonHabitats(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ...
    ("names[0].name","grottes"),
    ("pokemon_species[0].name","zubat"),
    ("pokemon_species[0].url","https://pokeapi.co/api/v2/pokemon-species/41/"),
])
