...
from pokeAPI_Models.Pokemon import Genders
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Pokemon/files/gender.json', 'r') as file:
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("id",1),
    ("name","female"),
    ("pokemon_species_details[0].pokemon_species.name","bulbasaur"),
    ("pokemon_species_details[0].pokemon_species.url","https://pokeapi.co/api/v2/pokemon-species/1/"),
    ("pokemon_species_details[0].rate",1),
    ("required_for_evolution[0].name","wormadam"),
    ("required_for_evolution[0].url","https://pokeapi.co/api/v2/pokemon-species/413/"),
