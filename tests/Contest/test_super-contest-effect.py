...
from pokeAPI_Models.Contests import SuperContestEffect
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Contest/files/super-contest-effect.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = SuperContestEffect(1)
    test2 = SuperContestEffect(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("appeal",2),
    ("flavor_text_entries[0].flavor_text","Enables the user to perform first in the next turn."),
    ("flavor_text_entries[0].language.name","en"),
    ("flavor_text_entries[0].language.url","https://pokeapi.co/api/v2/language/9/"),
    ("id",1),
    ("moves[0].name","agility"),
    ("moves[0].url","https://pokeapi.co/api/v2/move/97/"),
...
def test_data_check(mock_make_request, property_name, expected_value, mock_response_data):
    ...
    test_data = SuperContestEffect(1)
