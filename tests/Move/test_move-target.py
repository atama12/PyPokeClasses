...
from pokeAPI_Models.Moves import MoveTarget
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Move/files/move-target.json', 'r') as file:
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("id",1),
    ("descriptions[0].description","Eine spezifische F\u00e4higkeit.  Wie diese F\u00e4higkeit genutzt wird h\u00e4ngt von den genutzten F\u00e4higkeiten ab."),
    ("descriptions[0].language.name","de"),
    ("descriptions[0].language.url","https://pokeapi.co/api/v2/language/6/"),
    ("name","specific-move"),
    ("names[0].language.name","de"),
    ("names[0].language.url","https://pokeapi.co/api/v2/language/6/"),
    ("names[0].name","Spezifische F\u00e4higkeit"),
])
