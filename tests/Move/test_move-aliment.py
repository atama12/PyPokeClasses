...
from pokeAPI_Models.Moves import MoveAilments
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Move/files/move-aliment.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = MoveAilments(1)
    test2 = MoveAilments(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("id",1),
    ("moves[0].name","thunder-punch"),
    ("moves[0].url","https://pokeapi.co/api/v2/move/9/"),
    ("name","paralysis"),
    ("names[0].language.name","fr"),
    ("names[0].language.url","https://pokeapi.co/api/v2/language/5/"),
    ("names[0].name","Paralysie"),
])
