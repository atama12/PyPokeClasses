...
from pokeAPI_Models.Moves import MoveCategory
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Move/files/move-category.json', 'r') as file:
...
def test_api_connect():
    ...
    test1 = MoveCategory(1)
    test2 = MoveCategory(50000)
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("id",1),
    ("descriptions[0].description","No damage; inflicts status ailment"),
    ("descriptions[0].language.name","en"),
    ("descriptions[0].language.url","https://pokeapi.co/api/v2/language/9/"),
    ("moves[0].name","sing"),
    ("moves[0].url","https://pokeapi.co/api/v2/move/47/"),
    ("name","ailment"),
])
