...
from pokeAPI_Models.Items import ItemPocket
import json

# 1. モックデータをファイルから読み込むためのfixture
@pytest.fixture
def mock_response_data():
    with open('tests/Item/files/item-pocket.json', 'r') as file:
...
@pytest.mark.parametrize("property_name, expected_value", [
    ("categories[0].name","collectibles"),
    ("categories[0].url","https://pokeapi.co/api/v2/item-category/9/"),
    ("id",1),
    ("name","misc"),
    ("names[0].language.name","zh-Hant"),
    ("names[0].language.url","https://pokeapi.co/api/v2/language/4/"),
    ("names[0].name","\u9053\u5177"),
])
